import {Product, Clothing, Appliances} from '../../data/products.js';

describe('test suite: Product',()=>{
  let product;
  beforeEach(()=>{
    product = new Product({
      id: "e43638ce-6aa0-4b85-b27f-e1d07eb678c6",
      image: "images/products/athletic-cotton-socks-6-pairs.jpg",
      name: "Black and Gray Athletic Cotton Socks - 6 Pairs",
      rating: {
        stars: 4.5,
        count: 87
      },
      priceCents: 1090,
      keywords: [
        "socks",
        "sports",
        "apparel"
      ]
    })
  });
  it('has the correct properties', ()=>{
    expect(Object.keys(product).length).toEqual(7);
    expect(product.id).toEqual("e43638ce-6aa0-4b85-b27f-e1d07eb678c6");
    expect(product.keywords.length).toEqual(3);
  });

  it('does not contain extraInfoHTML, extraInfoHTML and extraInfoHTML', ()=>{
    expect(product.extraInfoHTML()).toEqual('');
    expect(product.extraInfoHTML()).toEqual('');
    expect(product.extraInfoHTML()).toEqual('');
  });

  it('get stars url',()=>{
    expect(product.getStarsUrl()).toEqual("images/ratings/rating-45.png")
  });

  it('get price url',()=>{
    expect(product.getPrice()).toEqual('$10.90')
  });
  });

  describe('test suite: Clothing',()=>{
    let clothing;
    beforeEach(()=>{
      clothing = new Clothing({
        id: "83d4ca15-0f35-48f5-b7a3-1ea210004f2e",
        image: "images/products/adults-plain-cotton-tshirt-2-pack-teal.jpg",
        name: "Adults Plain Cotton T-Shirt - 2 Pack",
        rating: {
          stars: 4.5,
          count: 56
        },
        priceCents: 799,
        keywords: [
          "tshirts",
          "apparel",
          "mens"
        ],
        type: "clothing",
        sizeChartLink: "images/clothing-size-chart.png"
      })
    });

  it('has all properties',()=>{
    expect(clothing.id).toEqual("83d4ca15-0f35-48f5-b7a3-1ea210004f2e");
    expect(clothing.sizeChartLink).toEqual("images/clothing-size-chart.png");
    expect(clothing.type).toEqual('clothing');
    expect(Object.keys(clothing).length).toEqual(8);
  });

  it('contains extraInfoHTML',()=>{
    expect(clothing.extraInfoHTML()).toContain(`<a href="images/clothing-size-chart.png" target="_blank">
    Size Chart
    </a>`)
    
  });
  });

  describe('test suite: Appliance',()=>{
    let appliance;
    beforeEach(()=>{
      appliance = new Appliances({
        id: "54e0eccd-8f36-462b-b68a-8182611d9add",
        image: "images/products/black-2-slot-toaster.jpg",
        name: "2 Slot Toaster - Black",
        rating: {
          stars: 5,
          count: 2197
        },
        priceCents: 1899,
        keywords: [
          "toaster",
          "kitchen",
          "appliances"
        ],
        type: "appliance",
        instructionsLink: "images/appliance-instructions.png",
        warrantyLink: "images/appliance-warranty.png"
    })
  });
  
  it('has all properties',()=>{
    expect(appliance.warrantyLink).toEqual("images/appliance-warranty.png");
    expect(appliance.instructionsLink).toEqual("images/appliance-instructions.png");
  });

  it('contains instruction inside extraInfoHTML',()=>{
    expect(appliance.extraInfoHTML()).toContain(`<a href="images/appliance-instructions.png" target="_blank">
  Instructions
  </a>`);
  });

  it('contains warranty inside extraInfoHTML',()=>{
    expect(appliance.extraInfoHTML()).toContain(`<a href="images/appliance-warranty.png" target="_blank">
  Warranty.
  </a>`);
  });


  it('gets the stars url', () => {

    expect(appliance.getStarsUrl()).toEqual('images/ratings/rating-50.png');
  });
  });